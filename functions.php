...
final class Sempa_Stock_Role
{
    ...
    public static function ensure_role_exists()
    {
        ...
        add_role(
            self::ROLE_KEY,
            __('Gestionnaire de Stock', 'sempa'),
            [
                'read' => true,
                'upload_files' => true,
            ]
...
final class Sempa_Login_Redirect
{
    ...
    public static function maybe_redirect($redirect_to, $requested_redirect_to, $user)
    {
        ...
        $allowed_roles = apply_filters('sempa_stock_redirect_roles', [Sempa_Stock_Role::ROLE_KEY]);
        if (!empty($allowed_roles) && array_intersect($allowed_roles, (array) $user->roles)) {
