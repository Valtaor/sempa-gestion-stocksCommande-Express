<?php
/**
 * Sempa theme custom functionality (version corrigée 2025-10-10)
 */
...
final class Sempa_Stock_Permissions
{
    ...
    // ✅ Correction ici : on rend les arguments optionnels
...
final class Sempa_Stock_Routes
{
    ...
    public static function get_movements(): WP_REST_Response
    {
        ...
        return rest_ensure_response(['movements' => $movements]);
    }
    ...
    public static function create_movement(WP_REST_Request $request)
    {
        ...
        if (!empty($product['is_kit']) && $type === 'out') {
            $components = $wpdb->get_results($wpdb->prepare(
                "SELECT kc.component_id, kc.quantity, p.name, p.stock 
                 FROM {$wpdb->prefix}kit_components kc 
                 JOIN {$wpdb->prefix}products p ON p.id = kc.component_id 
                 WHERE kc.kit_id = %d",
                $product_id
            ), ARRAY_A);
    ...
    public static function create_category(WP_REST_Request $request)
    {
        ...
        return rest_ensure_response(['status' => 'success', 'id' => (int)$wpdb->insert_id]);
    }
    ...
    private static function hydrate_components(array $product, \wpdb $wpdb): array
    {
        ...
        $product['components'] = $wpdb->get_results($wpdb->prepare(
            "SELECT p.id, p.name, p.reference, kc.quantity 
             FROM {$wpdb->prefix}kit_components kc 
             JOIN {$wpdb->prefix}products p ON p.id = kc.component_id 
             WHERE kc.kit_id = %d",
            (int)$product['id']
...
// ✅ Chargement différé de toutes les classes après setup du thème
